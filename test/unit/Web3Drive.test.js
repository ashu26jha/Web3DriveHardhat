...
!developmentChains.includes(network.name) ? describe.skip : describe ("Web3Drive Tests", function () {
    ...
    describe("Uploads file, and retrieves back", async function (){
        it("Uploads file", async function (){
            ...
            const filehashfromcontract = await web3contract.getIPFShash("Test.png");
            assert.equal("0x1234567891",filehashfromcontract);
        });

        it("Does not allow to upload files with same names", async function (){
            await web3contract.addFile("Test.png","0x1234567891");
            // Trying to upload same file name again.. Expecting an error
            await expect (web3contract.addFile("Test.png","0x1234567891")).to.be.revertedWith('fileNameAlreadyExist');
        });

        it("Checks event emitted when file added",async function (){
            const txResponse = await web3contract.addFile("Test.png","0x1234567891");
